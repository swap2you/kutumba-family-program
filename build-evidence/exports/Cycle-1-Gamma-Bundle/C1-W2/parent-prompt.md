# Gamma Prompt — C1-W2 Parent / Family Deck

## Deck identity

- **Module:** C1-W2 — I Am Not This Body
- **Audience:** Parents and adult family members
- **Purpose:** Teach body/self distinction with source grounding and practical application
- **Format:** 16:9 presentation
- **Recommended card count:** 12
- **Style:** Calm, scriptural, practical
- **Theme:** Changing body, continuing self
- **Source pack:** sources.yaml, VERSE-AND-REFERENCE-STUDY.md

## Global instructions

Paste everything below into Gamma. Do not add invented quotes.

---

## Card plan

### Card 1 — Title

**Content:**  
C1-W2 — I Am Not This Body  
Cycle 1 · Identity and the Human Problem  
KUTUMBA Family Program

**Visual:** ```[ASSET: visuals/rendered/c1-w2-concept-life-stages.svg]```  
**Speaker note:** Welcome families; state this is protected hearing time, not entertainment.
**Source:** KUTUMBA module README

---

### Card 2 — Essential question

**Content:**  
What changes in the body?  
What continues in the self?  
How should we live?

**Visual:** ```[ASSET: visuals/rendered/c1-w2-concept-life-stages.svg]```  
**Speaker note:** Read the essential question slowly; invite silent reflection before answers.
**Source:** RESEARCH-DOSSIER.md

---

### Card 3 — The old family photo

**Content:**  
A child finds old photos: "That can't be you!"  
Everyone laughs — but who is the "me" in every picture?

**Visual:** ```[ASSET: visuals/rendered/c1-w2-concept-life-stages.svg]```  
**Speaker note:** Use opening hook as modern door only — pivot quickly to śāstra.
**Source:** prem-ki-katha.md

---

### Card 4 — BG 2.13 summary

**Content:**  
**Bhagavad-gītā 2.13** (KUTUMBA summary)  
The embodied soul passes through bodily stages — childhood, youth, and old age. A sober person is not bewildered by this change.

**Visual:** Life-stage timeline (import from process-flow.mmd)  
**Speaker note:** Display verse link; paraphrase only; no invented purport language.
**Source:** https://vedabase.io/en/library/bg/2/13/

---

### Card 5 — deha and dehī

**Content:**  
**deha** = the body (changes)  
**dehī** = the embodied self (continues)  
Precise language: "I **have** a body."

**Visual:** Two-column diagram from concept-map.mmd  
**Speaker note:** Tell one kathā beat from prem-ki-katha narrative section; keep under 2 minutes.
**Source:** BG 2.13, BG 13.1 summary

---

### Card 6 — Garment analogy

**Content:**  
**BG 2.22** — As one changes worn garments, the soul accepts new bodies.  
**Today:** costume relay — outfit changes, person stays.

**Visual:** ```[ASSET: visuals/rendered/c1-w2-concept-life-stages.svg]```  
**Speaker note:** Clarify means vs does-not-mean; stop if debate becomes argument.
**Source:** https://vedabase.io/en/library/bg/2/22/

---

### Card 7 — What this means / does not mean

**Content:**  
| Means | Does not mean |
| Care for body as instrument | Neglect medicine or sleep |
| Same self through change | Mock disability |
| Reduce vanity | Dismiss grief or pain |

**Visual:** ```[ASSET: visuals/rendered/c1-w2-analogy-driver-vehicle.svg]```  
**Speaker note:** Case study is anonymized; no forced personal disclosure.
**Source:** MISCONCEPTIONS-AND-BOUNDARIES.md

---

### Card 8 — Case study trio

**Content:**  
1. Teen + social media appearance  
2. Parent fearing aging  
3. Devotee skipping sleep/medicine  

**Task:** Identify mistaken conclusion + compassionate response.

**Visual:** Three icons (phone, calendar, moon)  
**Speaker note:** Bhakti lab preview — demonstrate one step only.
**Source:** research/CONTEMPORARY-APPLICATIONS.md

---

### Card 9 — Positive identity

**Content:**  
**Caitanya-caritāmṛta Madhya 20.108** (summary)  
Constitutional identity: eternal servant of Kṛṣṇa.  
"I am not only this body — I am Kṛṣṇa's servant."

**Visual:** ```[ASSET: visuals/rendered/c1-w2-concept-life-stages.svg]```  
**Speaker note:** Home practice: one realistic step; write it down.
**Source:** https://vedabase.io/en/library/cc/madhya/20/108/

---

### Card 10 — Language practice

**Content:**  
Rewrite:  
- "I am ugly" → "I struggle with appearance; I serve Kṛṣṇa"  
- "I am my job" → "My service role today is…"  
- "I am too old" → "This body has years; wisdom serves"

**Visual:** Before/after text blocks  
**Speaker note:** Newcomer path: glossary slip + opt-in participation.
**Source:** parent-lesson.md

---

### Card 11 — Identity pause

**Content:**  
1. Three slow breaths — who notices?  
2. Recall: "I am Kṛṣṇa's servant; this body is for service."  
3. Optional: 5 minutes chanting

**Visual:** Seated figure silhouette, peaceful  
**Speaker note:** Safeguarding: two adults visible; honor opt-out.
**Source:** bhakti-lab.md

---

### Card 12 — Home practice + source citation

**Content:**  
**This week:** Identity pause at one daily trigger.  
**Sources:** BG 2.13, 2.16, 2.20, 2.22, 13.1–3 · SB 10.84.13 · CC Madhya 20.108–109  
Full text: vedabase.io

**Visual:** QR or URL list (no book covers)  
**Speaker note:** Close with gratitude and next-week pointer only.
**Source:** sources.yaml

---

## Citation card (append or footer on all decks)

KUTUMBA summaries only. Full purports: Bhaktivedanta Book Trust via VedaBase. No unauthorized reproduction.

## Final review checklist

- [ ] 12 cards present
- [ ] Safeguarding slide included
- [ ] No invented śāstra
- [ ] No BBT images
