# Gamma Prompt — C1-W2 Kiśora–Kiśorī Deck

## Deck identity

- **Module:** C1-W2
- **Audience:** Kiśora–Kiśorī ages 9–14 (core 9–11 + challenge 12–14)
- **Purpose:** Text observation, critical application, service identity
- **Format:** 16:9
- **Recommended card count:** 11
- **Style:** Clean, discussion-ready
- **Theme:** Observe, distinguish, serve

---

## Card plan

### Card 1 — Title

**Content:**  
I Am Not This Body  
Kiśora–Kiśorī · C1-W2  
Observe · Distinguish · Serve

**Visual:** Split screen: mirror vs. heart icon  
**Speaker note:** Welcome families; state this is protected hearing time, not entertainment.

---

### Card 2 — Essential question

**Content:**  
What **changes**?  
What **continues**?  
What should we **do**?

**Visual:** concept-map.mmd simplified  
**Speaker note:** Read the essential question slowly; invite silent reflection before answers.

---

### Card 3 — BG 2.13 observation

**Content:**  
**Bhagavad-gītā 2.13** (KUTUMBA summary)  
The soul passes through childhood, youth, and old age. The sober are not bewildered.

**Observe:**  
1. Three stages named?  
2. Who passes through?  
3. What attitude?

**Visual:** Timeline + three questions  
**Source:** https://vedabase.io/en/library/bg/2/13/

---

### Card 4 — Changes vs continues

**Content:**  
| Changes | Continues |
| Height, hair, injuries | Awareness |
| Mood swings (expression) | Responsibility |
| Social labels | Service to Kṛṣṇa |

**Visual:** Two-column table  
**Speaker note:** Display verse link; paraphrase only; no invented purport language.
**Source:** https://vedabase.io/en/library/bg/13/1/

---

### Card 5 — Social media case

**Content:**  
**Case CS-01** (anonymized)  
Hours editing photos. Mood crashes with likes.  
**Mistaken idea:** Profile = whole self

**Visual:** ```[ASSET: visuals/rendered/c1-w2-concept-life-stages.svg]```  
**Speaker note:** Tell one kathā beat from prem-ki-katha narrative section; keep under 2 minutes.
**Source:** CONTEMPORARY-APPLICATIONS.md

---

### Card 6 — Device / user analogy

**Content:**  
Profile picture changes like a costume.  
**You** are still the person Kṛṣṇa knows.  
Online harm still hurts — pixels don't cancel kindness.

**Visual:** Avatar icons with arrow to person icon  
**Speaker note:** Clarify means vs does-not-mean; stop if debate becomes argument.

---

### Card 7 — Disability & dignity

**Content:**  
**BG 2.20** (summary) — Soul not destroyed when body is injured.  
Mocking is never OK. Include · accommodate · respect.

**Visual:** Inclusion symbols  
**Speaker note:** Case study is anonymized; no forced personal disclosure.
**Source:** https://vedabase.io/en/library/bg/2/20/

---

### Card 8 — Debate motion

**Content:**  
**Motion:** "Because we are not the body, bodily boundaries don't matter."  
**Rules:** Ideas not people · cite source · facilitator can pause

**Visual:** Two podiums  
**Speaker note:** Bhakti lab preview — demonstrate one step only.
**Source:** MISCONCEPTIONS-AND-BOUNDARIES.md

---

### Card 9 — Reflective writing

**Content:**  
Prompt (private, optional share):  
"When I remember I am not only my appearance, I will…"

**Visual:** Lined card graphic  
**Speaker note:** Home practice: one realistic step; write it down.

---

### Card 10 — Service identity

**Content:**  
**CC Madhya 20.108** (summary)  
Eternal servant of Kṛṣṇa.  
Template: "This body can be used to ___ this week."

**Visual:** ```[ASSET: visuals/rendered/c1-w2-concept-life-stages.svg]```  
**Source:** https://vedabase.io/en/library/cc/madhya/20/108/

---

### Card 11 — Sources & home link

**Content:**  
Key verses: BG 2.13, 2.16, 2.20, 13.1 · CC Madhya 20.108  
vedabase.io  
Join family identity pause tonight.

**Visual:** URL list  
**Speaker note:** Safeguarding: two adults visible; honor opt-out.

---

## Citation card

KUTUMBA summaries. Full verses on VedaBase. Case studies anonymized pedagogy — not śāstra.

## Final checklist

- [ ] Debate safeguards stated
- [ ] No forced disclosure prompts
- [ ] No transmigration deep-dive
